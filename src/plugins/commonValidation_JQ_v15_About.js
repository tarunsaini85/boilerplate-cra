...
window.commonValidator = function() {
    ...
    var prv = {
        validate: function(params) {
            var c = prv;
            c.onErr = params.onError || null;
            c.onValid = params.onValid || null;
            ...
            if (params.messageBox) {
                var mBoxId = params.messageBox.id || null,
                    mBoxCnt = params.messageBox.content || null,
                    mBoxHid = params.messageBox.hideOthers || false;
            } else {
                var mBoxId, mBoxCnt = null;
            }
            if (params.styles) {
            ...
            var fNames = params.formNames || null,
                dEvts = params.defaultEvents || null;
            var sBts = params.submitButton || null;
            var delay = params.fireDelay || 0;
            if (fNames.constructor === Array) {
                for (var x = 0; x < fNames.length; x++) {
                    c.validInit(fNames[x], sBts, errClass, okClass, sOkClass, parentExist, parentLevel, mBoxId, mBoxCnt, mBoxHid, inlineErr, dEvts, delay);
                }
            } else {
                c.validInit(fNames, sBts, errClass, okClass, sOkClass, parentExist, parentLevel, mBoxId, mBoxCnt, mBoxHid, inlineErr, dEvts, delay);
            }
        },

        validInit: function(fN, sB, eC, oC, soC, pE, pL, mBoxId, mBoxCnt, mBH, inErr, dEvts, delay) {
            var c = prv,
                z, frmElm = $('form[name=' + fN + ']').get(0),
                sBtns, rStat, noVldSt = false,
                sbtFlg;
            if (!frmElm) {
                var e = new Error(fN);
                e.name = "CommonValidator Form";
                throw e;
            }
            c.fName = fN;
            c[fN] = {
            ...
            $(frmElm).submit(function() {
                ...
                if (parseInt(delay) === 0) {
                    ...
                    if (c.befSbt) {
                        c.befSbt({ err: c.erArryExt, formName: fN, isSubmit: submitTriggered })
                    }
        ...
        setDefaultValues: function(fElm) {
            var c = prv,
                obj = (fElm) ? $(fElm) : $('form[name=' + c.fName + ']'),
                x, fEls = c.getFrmElms($(obj), true);
        ...
        checkSubmit: function(fElm, sB, eC, oC, soC, mBoxId, mBoxCnt, mBH, inErr) {
            var c = prv,
                s, x, vlds, cV, chkVld = false,
                sbtCallB, fEls = c.getFrmElms($(fElm), false),
                sBtns = c.getSbtBtns($(fElm), sB);
            (mBH) ? $('.mgBox').hide(): '';
            $(fElm).attr('chk', false);
            c.erArry = {};
            c.erArryExt = {};
            ...
            if (c.lastErr && $(fElm).attr('chk') == 'true') {
                if (mBoxId) {
                    ...
                    if (mBoxCnt) {
                        ...
                        for (y in c.erArry) {
                            cVeR.push(c.erArry[y]);
                        }
                        if (mBoxCnt.customContent) {
                            var cstMsg = mBoxCnt.customContent,
                                cP = $('<p>');
                            cP.html(cstMsg);
                            msgDiv.append(cP);
                        }
                ...
                if (c.eFocus) {
                    (c.lastErr.obj) ? $(c.lastErr.objCont).focus(): c.lastErr.focus();
                }
            } else {
                (mBoxId) ? $('#' + mBoxId).hide(): '';
            }
        ...
        isValid: function(id) {
            var c = prv,
                obj = (id) ? $('#' + id) : $('form[name=' + c.fName + ']'),
                x, vlds, cV, chkVld = false;
        ...
        checkValids: function(vlds, obj, e, fElm, eC, oC, soC, inErr) {
            ...
            if (arguments.length == 2) {
                ...
                if (e.keyCode != 9 && e.keyCode != 16 && e.keyCode != 17 && e.keyCode != 18 && e.keyCode != 35 && e.keyCode != 36 && e.keyCode != 27 && e.keyCode != 20 && e.keyCode != 13) {
                    evtChk = 1
                }
            }
            if (evtChk == 1) {
                    ...
                    if (!vRet) {
                        switch (vds[x].split(':')[0]) {
                            case 'softReq':
                                ...
                                if (argChk) {
                                    (argSts) ? '' : argSts = c.validators.reqChk(obVal, vds[x].split(':')[1]);
                                } else {
                            ...
                            case 'alphaDS':
                                if (obj.attr('placeholder') && obj.val() == obj.attr('placeholder')) {
                                    ...
                                    if (argChk) {
                                        (argSts) ? '' : argSts = c.validators.alphadsChk(obj.val(), vds[x].split(':')[1]);
                                    } else {
                            ...
                            case 'alpha':
                                if (obj.attr('placeholder') && obj.val() == obj.attr('placeholder')) {
                                    ...
                                    if (argChk) {
                                        (argSts) ? '' : argSts = c.validators.alphaChk(obj.val(), vds[x].split(':')[1]);
                                    } else {
                            ...
                            case 'num':
                                if (obj.attr('placeholder') && obj.val() == obj.attr('placeholder')) {
                                    ...
                                    if (argChk) {
                                        (argSts) ? '' : argSts = c.validators.numChk(obj.val(), vds[x].split(':')[1]);
                                    } else {
                            ...
                            case 'float':
                                if (obj.attr('placeholder') && obj.val() == obj.attr('placeholder')) {
                                    ...
                                    if (argChk) {
                                        (argSts) ? '' : argSts = c.validators.floatChk(obj.val(), vds[x].split(':')[1]);
                                    } else {
                            ...
                            case 'alphanum':
                                if (obj.attr('placeholder') && obj.val() == obj.attr('placeholder')) {
                                    ...
                                    if (argChk) {
                                        (argSts) ? '' : argSts = c.validators.alphanumChk(obj.val(), vds[x].split(':')[1]);
                                    } else {
                            ...
                            case 'email':
                                if (obj.attr('placeholder') && obj.val() == obj.attr('placeholder')) {
                                    ...
                                    if (argChk) {
                                        (argSts) ? '' : argSts = c.validators.emailChk(obj.val(), vds[x].split(':')[1]);
                                    } else {
                            ...
                            case 'specialChar':
                                if (obj.attr('placeholder') && obj.val() == obj.attr('placeholder')) {
                                    ...
                                    if (argChk) {
                                        (argSts) ? '' : argSts = c.validators.splChk(obj.val(), vds[x].split(':')[1]);
                                    } else {
                            ...
                            case 'charRange':
                                if (obj.attr('placeholder') && obj.val() == obj.attr('placeholder')) {
                                    ...
                                    var o = obj,
                                        minL = (o.attr('minL')) ? o.attr('minL') : o.attr('minlength'),
                                        maxL = (o.attr('maxL')) ? o.attr('maxL') : o.attr('maxlength'),
                                        scop = '';
                                    (!o.attr('scope') || o.attr('scope') == '') ? scop = 'in': scop = o.attr('scope');
                                    if (argChk) {
                                        (argSts) ? '' : argSts = c.validators.rangeChk(o.val(), minL, maxL, scop);
                                    } else {
                            ...
                            case 'valRange':
                                if (obj.attr('placeholder') && obj.val() == obj.attr('placeholder')) {
                                    ...
                                    var o = obj,
                                        minV = (o.attr('minval')) ? parseFloat(o.attr('minval')) : parseFloat(o.attr('minV')),
                                        maxV = (o.attr('maxval')) ? parseFloat(o.attr('maxval')) : parseFloat(o.attr('maxV')),
                                        scop = '';
                                    (!o.attr('scope') || o.attr('scope') == '') ? scop = 'in': scop = o.attr('scope');
                                    if (argChk) {
                                        (argSts) ? '' : argSts = c.validators.rangeVChk(o.val(), minV, maxV, scop);
                                    } else {
                            ...
                            case 'checked':
                                if (argChk) {
                                    if (obj.attr('type') == 'checkbox') {
                                        var fElm, fPar = obj;
                                        while (fPar.get(0).nodeName.toLowerCase() != 'form') {
                                            fPar = fPar.parent();
                                        }
                                    ...
                                    if (obj.attr('type') == 'checkbox') {
                                        vRet = c.validators.checkedChk(obj, fElm);
                                    } else if (obj.attr('type') == 'radio') {
                                        vRet = c.validators.checkedRadChk(obj, fElm);
                                    }
                                    vCode = vds[x].split(':')[1];
                                }
                                break;
                            ...
                            case 'selected':
                                if (argChk) {
                                    (argSts) ? '' : argSts = c.validators.selectedChk(obj);
                                } else {
                ...
                if (argChk) {
                    ...
                    if (vRet && !sReq) {
                        c.heighlightErrOk(vCode, obj, fElm, 'err', eC, oC, soC, inErr);
                        c.lastErr = obj;
                        return false;
                    }
                    if (vRet && sReq) {
                        c.heighlightErrOk(vCode, obj, fElm, 'sMnd', eC, oC, soC, inErr);
                        c.lastErr = c.lastErr;
                        return true;
                    } else {
                        c.heighlightErrOk(vCode, obj, fElm, 'ok', eC, oC, soC, inErr);
                        return true;
                    }
                }
            }
        ...
        heighlightErrOk: function(vCode, obj, fElm, hTyp, eC, oC, soC, inErr) {
            var c = prv,
                x, errLbl = null,
                errM = c.errs[vCode] || vCode,
                errI = obj.attr('id') + '_err',
                errN = obj.attr('name') + '_err',
                frLbs = $(fElm).find('.erLbl'),
                eArrIndx = obj.attr('id') || obj.attr('name'),
                errF = null,
                fN = $(fElm).attr('name');
            ...
            if (hTyp == 'err') {
                ...
                if (errLbl) {
                    var minL = obj.attr('minL') || obj.attr('minlength'),
                        maxL = obj.attr('maxL') || obj.attr('maxlength'),
                        minV = obj.attr('minV') || obj.attr('minval'),
                        maxV = obj.attr('maxV') || obj.attr('maxval'),
                        errLblId = errLbl.attr('id');
                    if (obj.val()) {
                        if (obj.val().indexOf('$') >= 0) {
                            var objVal = obj.val().replace(/\$/g, '$$$');
                        }
                    }
                    errM = errM.replace('[MinL]', minL).replace('[MaxL]', maxL).replace('[MinV]', minV).replace('[MaxV]', maxV).replace('[currVal]', (objVal) ? objVal.toString().replace(/</g, '&lt;').replace(/>/g, '&gt;') : '');
                    (inErr) ? errLbl.html(errM): '';
                    c.erArry[eArrIndx] = errM;

                    _selector = obj.attr('name') || obj.attr('id') || obj.get(0).nodeName;
                    c.erArryExt[_selector] = errM;
                }
                $(fElm).attr('chk', 'true');

                if (!submitTriggered && c.onErr && !$.isEmptyObject(c.erArryExt)) {
                    c.onErr({ err: c.erArryExt, formName: fElm.name, node: obj[0], isSubmit: submitTriggered });
                }
            } else if (hTyp == 'ok') {
                if (parent && parent.length > 0) {
                ...
                if (!submitTriggered && c.onValid) {
                    _selector = obj.attr('name') || obj.attr('id') || obj.get(0).nodeName;
                    c.erArryExt[_selector] = null;
                    c.onValid({ err: c.erArryExt, formName: fElm.name, node: obj[0], isSubmit: submitTriggered });
                }
            } else if (hTyp == 'sMnd') {
                if (parent && parent.length > 0) {
                ...
                if (errLbl) {
                    var minL = obj.attr('minL') || obj.attr('minlength'),
                        ...
                        maxV = obj.attr('maxV') || obj.attr('maxval'),
                        errLblId = errLbl.attr('id');
                    errM = errM.toString().replace('[MinL]', minL).replace('[MaxL]', maxL).replace('[MinV]', minV).replace('[MaxV]', maxV).replace('[currVal]', obj.val().replace(/</g, '&lt;').replace(/>/g, '&gt;'));
                    (inErr) ? errLbl.html(errM): '';
                    c.erArry[eArrIndx] = errM;
                }
            } else if (hTyp == 'rem') {
        ...
        validators: {
            reqChk: function(val, vCd) {
                var regX = /^\s*$/;
                if (prv.errs[vCd]) {
                    regX = new RegExp(prv.errs[vCd].regEx || regX);
                }
            ...
            checkedRadChk: function(elm, frm) {
                var x, f = false,
                    fElm = $(frm).find("input[type=radio]");
            ...
            checkedChk: function(elm, frm) {
                var x, f = false,
                    fElm = $(frm).find("input[type=checkbox]");
                for (x = 0; x < fElm.length; x++) {
                    if (fElm.eq(x).attr('name') == elm.attr('name')) {
                        (fElm.eq(x).is(':checked')) ? f = true: '';
                    }
        ...
        getSbtBtns: function(frmElm, sB) {
            var x, y, els = [],
                sBtns = frmElm.find("input[type=submit], button[type=submit]");
            if (sBtns.length > 0) {
                els.push(sBtns);
            }
            if (sB) {
        ...
        processAltRel: function(eId, stat) {
            var elm = $('#' + eId);
            var altRel = elm.attr('altrel');
            if (stat == 's') {
                (altRel) ? elm.attr('rel', altRel): '';
            } else if (stat == 'h') {
                (altRel) ? '' : elm.attr('altrel', elm.attr('rel'));
                elm.removeAttr('rel');
            }
        },
    ...
    return {
        ...
        getErrorStatus: prv.getErrorStatus
    }
};
/*******End of commonValidator*/


// format code : ctrl+Shift+H (using sublime)

//v11 : By saeed: Fixed class toggle on error behaviour
/*eslint-enable*/